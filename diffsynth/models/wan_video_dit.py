...
def precompute_freqs_cis_4d(
    ...
    print(f"  - Allocating dims -> Shot ID: {dim_shot}, Time: {dim_time}, H: {dim_h}, W: {dim_w}")
...
class AttentionModule(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None,shot_latent_indices=None,per_g=0):
        if attn_mask is not None:
            ...
            x = attention_per_batch_with_shots(q=q, k=k, v=v, shot_latent_indices=shot_latent_indices, num_heads=self.num_heads,per_g=per_g)
        else:
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs, attn_mask=None,shot_latent_indices=None,per_g=0):
        ...
        x = self.gate(x, gate_msa, self.self_attn(input_x, freqs,shot_latent_indices=shot_latent_indices,per_g=per_g))
...
class WanModel(torch.nn.Module):
    ...
    def forward(self,
        ...
        if self.shot_embedding is not None and shot_indices is not None:
            assert shot_indices.shape == (x.shape[0], f), f"Shot indices shape mismatch. Expected (batch, {f}), got {shot_indices.shape}"
